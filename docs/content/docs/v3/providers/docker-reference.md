...
Quick reference for every Docker label TSDProxy supports. For detailed explanations, see the [Docker provider page]({{< ref "/docs/v3/providers/docker" >}}).
...
| Label | Default | Description |
|-------|---------|-------------|
| `tsdproxy.domain` | — | Custom domain for this proxy (e.g. `app.example.com`) |
| `tsdproxy.dnsprovider` | `defaultDNSProvider` | DNS provider name from `tsdproxy.yaml` |
| `tsdproxy.tlsprovider` | `defaultTLSProvider` | TLS provider name from `tsdproxy.yaml` |

See [Custom Domains]({{< ref "/docs/v3/advanced/custom-domains" >}}) for full setup instructions.
...
These labels still work but are deprecated. See the [legacy section on the Docker provider page]({{< ref "/docs/v3/providers/docker#legacy-labels-v1" >}}) for details.
