...
when a container restarts and gets a new IP. See [Health Check]({{< ref "/docs/v3/operations/health-check#backend-health-monitoring" >}}) for details.
...
Assigns a custom domain to the proxy. When set, TSDProxy creates a DNS CNAME
record and provisions a TLS certificate for the domain. Requires a matching DNS
and TLS provider in `tsdproxy.yaml`. See [Custom Domains]({{< ref "/docs/v3/advanced/custom-domains" >}}) for full setup.
...
{{% /details %}}
{{% details title="tsdproxy.dnsprovider" %}}

Overrides the DNS provider for this container. The value must match a provider
name defined in the `dnsProviders` section of `tsdproxy.yaml`. If not set, the
`defaultDNSProvider` from the config is used.
...
{{% /details %}}
{{% details title="tsdproxy.tlsprovider" %}}

Overrides the TLS provider for this container. The value must match a provider
name defined in the `tlsProviders` section of `tsdproxy.yaml`. If not set, the
`defaultTLSProvider` from the config is used.
...
Example with all custom domain labels:
...
icon based on the image name. See available icons in [icons]({{< ref "/docs/v3/advanced/icons" >}}).
