...
- **`preventDuplicates` tri-state** — the `preventDuplicates` option now accepts `"auto"` in addition to `"true"` and `"false"`. When set to `"auto"`, duplicate prevention is enabled automatically when OAuth credentials are configured. See [Prevent Duplicate Machines]({{< ref "/docs/v3/advanced/tailscale#prevent-duplicate-machines" >}}) for details.
