...
See the [introduction]({{< ref "/docs/v3/_index" >}}) for a full overview.
...
[upgrading guide]({{< ref "/docs/v3/upgrading/from-v1" >}}) for migration details.

## Can I use TSDProxy without Docker?

Yes. You can run TSDProxy as a standalone binary and use the
[Lists provider]({{< ref "/docs/v3/providers/lists" >}}) to define proxies by
IP address or hostname. See the
[standalone deployment]({{< ref "/docs/v3/deployment/standalone" >}}) page for
instructions.
...
[Authentication Methods]({{< ref "/docs/v3/security/auth-methods" >}}) for a
comparison.
...
proxy in the [dashboard]({{< ref "/docs/v3/advanced/dashboard" >}}) to
authenticate manually. Check the logs for specific errors.

## Why is my proxy stuck at "Awaiting Approval"?

The proxy has registered with Tailscale, but an admin needs to approve the
machine in the Tailscale admin console. Go to
[https://login.tailscale.com/admin/machines](https://login.tailscale.com/admin/machines)
and approve the machine. This status is different from "Authenticating" — the
node has valid credentials, but your Tailscale ACL policy requires admin
approval for new machines.
...
[Docker provider]({{< ref "/docs/v3/providers/docker" >}}) docs for details.
...
[host network mode]({{< ref "/docs/v3/advanced/host-mode" >}}) for examples.

## Can I proxy services not running in Docker?

Yes. The [Lists provider]({{< ref "/docs/v3/providers/lists" >}}) lets you
...
Yes, using [Tailscale Funnel]({{< ref "/docs/v3/security/funnel" >}}). Add the
...
[TCP Proxy & SSH]({{< ref "/docs/v3/advanced/tcp-proxy" >}}) for configuration
examples.
...
`http://127.0.0.1:8080`). The [dashboard]({{< ref "/docs/v3/advanced/dashboard" >}})
page has step-by-step instructions for both methods.

## Can I run multiple TSDProxy instances?

Yes. Check the [scenarios]({{< ref "/docs/v3/scenarios" >}}) section for common
...
See the [tags section]({{< ref "/docs/v3/advanced/tailscale#tags" >}}) for
details.
...
[roadmap]({{< ref "/docs/v3/advanced/roadmap#multi-target-load-balancing" >}}).
...
> [prevent duplicates section]({{< ref "/docs/v3/advanced/tailscale#prevent-duplicate-machines" >}})
> before enabling it.
