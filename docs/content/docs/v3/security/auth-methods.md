...
detailed setup instructions, see [Tailscale configuration]({{< ref "/docs/v3/advanced/tailscale" >}}).
...
| Fresh key on restart | Yes | N/A | No |
| Duplicate prevention | Yes | No | No |
| Device reconciliation | Yes | No | No |
...
OAuth is recommended for production deployments because it enables API-backed
features like fresh one-time keys on each restart, duplicate device prevention
(`preventDuplicates`), and automatic stale device reconciliation.
